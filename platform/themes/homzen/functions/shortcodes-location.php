    ...
    Shortcode::register('location', __('Location'), __('Location'), function (ShortcodeCompiler $shortcode) {
        ...
        $locations = match ($shortcode->type) {
            'city' => City::query()
                ...
                ->orderBy('name')
                ->get()
                ->when(is_plugin_active('real-estate'), function (Collection $collection) use ($destination) {
                    $collection->transform(fn (City $city) => $city->setAttribute('url', route("public.$destination-by-city", $city->slug)));
                }),
            'state' => State::query()
                ->wherePublished()
                ->whereIn('id', $stateIds)
                ...
                ->get()
                ->when(is_plugin_active('real-estate'), function (Collection $collection) use ($destination) {
                    $collection->transform(fn (State $state) => $state->setAttribute('url', route("public.$destination-by-state", $state->slug)));
                }),
            default => collect(),
        };
